...
class SignUpViewControllerTests: XCTestCase {
    ...
    override func setUp() {
        super.setUp()
        window = UIWindow()
        let storyboard = UIStoryboard(name: "SignUpView", bundle: Bundle(for: SignUpViewController.self))
    ...
    func testSaveStoreProfile() {
        ...
        viewController.saveStoreProfile()
        
        DispatchQueue.main.asyncAfter(deadline: .now() + 1) {
            XCTAssertNotNil(StoreManager.shared.store)
            XCTAssertEqual(StoreManager.shared.store?.name, "Test Store")
            expectation.fulfill()
        }
